...
curl --silent \
  ...
  https://api.github.com/repos/blacknaut/tema/releases/latest |
  grep '"tag_name":' |
  sed -E 's/.*"([^"]+)".*/\1/'
}
...
mkdir -p $PTERO/temp
curl -sSLo $PTERO/temp/BackgroundVideo.tar.gz https://raw.githubusercontent.com/blacknaut/tema/BackgroundVideo.tar.gz
